    ...
    try:
        ...
        if work_history:
            ...
            for job in work_history:
                ...
                story.append(Paragraph(f"{job.get('start_date', '')} - {job.get('end_date', 'Present')}", styles["Italic"]))
                if job.get("summary"):
        ...
        story.append(HRFlowable(width="100%", thickness=1, color=colors.lightgrey))
        story.append(Spacer(1, 0.1 * inch))
        story.append(Paragraph("Generated by <b>Tenabot AI Resume Assistant</b> using Gemini AI.", styles["Italic"]))
