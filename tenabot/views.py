...
def register_telegram_user(request):
    """
    Handles Telegram WebApp login:
    - Validates initData signature
    - Parses Telegram user payload
    ...
    if not init_data:
        return Response({"detail": "initData missing"}, status=status.HTTP_400_BAD_REQUEST)

    # ✅ verify Telegram signature
    if not check_telegram_data_integrity(init_data):
        return Response({"detail": "Invalid Telegram signature"}, status=status.HTTP_403_FORBIDDEN)

    # ✅ Parse user JSON from initData string
    try:
        data_dict = dict(item.split("=", 1) for item in init_data.split("&") if "=" in item)
        telegram_user = json.loads(data_dict.get("user"))
    except Exception:
        return Response({"detail": "Invalid user payload"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    if not telegram_id:
        return Response({"detail": "Telegram user id missing"}, status=400)
    ...
    return Response(
        {
            "detail": "User registered" if created else "User logged in",
            "user": {
                "id": user.id,
